...
- **The format has no codegen step; the SERVICE does.** Keep these apart.
  `formats/sitemap.ebnf` is data, compiled to a descriptor on demand — proto-sitemap
  generates and commits no proto for the sitemap *format*, and `build.sh` only sets
  up and builds. That is unchanged.
  `proto/sitemap_service.proto` is the one exception, added when `cmd/sitemap-svc`
  became a gRPC service (2026-08-28): it describes the service contract — what a
  caller sends to ask for a walk and what comes back — and nothing about the
  sitemap format. Its Go bindings are regenerated by **`./regen.sh`** into
  `proto/pb/` and committed, so an ordinary build still needs no protoc. This is
  exactly the "add a `regen.sh` and document it here" path the previous rule
  called for. Never describe the sitemap format in a `.proto`; the typed AST is
  compiled from the grammar and never appears on the wire.
...
- **Two gates.**
  - `go test ./...` is self-contained: `service/sitemap_test.go` (grammar compiles,
    ...
    `cmd/sitemap-svc/expand_test.go` gates the walk over `httptest` origins, and
    `TestGRPCSurface` additionally serves the real gRPC surface over a socket
    (reflection, health, and an InvalidArgument for an empty request):
